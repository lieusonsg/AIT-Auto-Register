"""Auto-push accounts to Any2API instance after registration.

After configuring the Any2API address and admin password in global settings,
each successful registration will automatically push the account to Any2API,
eliminating the need for manual export/import.
"""
...
class Any2ApiClient:
    ...
    def _login(self) -> bool:
        try:
            ...
            logger.warning(f"[Any2API] Login exception: {exc}")
            return False
...
def _get_any2api_config() -> tuple[str, str]:
    """Read Any2API address and password from global configuration."""
    try:
...
def push_account_to_any2api(account: Any, *, log_fn=None) -> bool:
    """Auto-push account to Any2API after registration.
