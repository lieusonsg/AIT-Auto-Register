...
export function getCaptchaStrategyLabel(executorType: string, policy?: CaptchaPolicy, providers?: ProviderOption[]) {
  if (executorType === 'headless' || executorType === 'headed') {
    ...
    return label ? `Browser mode defaults to ${label}` : 'Browser mode has no default captcha provider configured'
  }
  const order = policy?.protocol_order || []
  if (order.length === 0) {
    return 'Protocol mode has no remote captcha provider configured'
  }
  const labels = order.map(value => providers?.find(item => item.value === value)?.label || value)
  return `Protocol mode auto-selects remote captcha service in enabled order: ${labels.join(' -> ')}`
}
