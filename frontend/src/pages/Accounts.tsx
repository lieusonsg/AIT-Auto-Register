    ...
    try {
      ...
      if (selection.identityProvider === 'mailbox') {
        if (!defaultMailboxProvider?.provider_key) {
          throw new Error('Default mailbox provider not configured. Please go to Settings to enable a mailbox provider')
        }
            ...
            configLoading ? (
              ...
              <>
                <div>
                  ...
                  <div className="mt-1 text-sm font-semibold text-[var(--text-primary)]">Select Registration Identity</div>
                  <div className="mt-1 text-xs text-[var(--text-muted)]">Shows what the current platform supports, so you don't need to research platform capabilities beforehand.</div>
                ...
                <div>
                  ...
                  <div className="mt-1 text-sm font-semibold text-[var(--text-primary)]">Select Execution Method</div>
                  <div className="mt-1 text-xs text-[var(--text-muted)]">All methods run automatically, differing only in protocol or browser channel.</div>
                ...
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] px-4 py-3 text-xs text-[var(--text-secondary)]">
                  ...
                  {selection.identityProvider === 'oauth_browser' && !reusableBrowser && (
                    <div className="mt-2 text-amber-400">Background browser automation requires Chrome Profile or Chrome CDP. Without configuration, only visible browser automation is allowed.</div>
                  )}
                </div>
        ...
        <div className="relative overflow-hidden border-b border-[var(--border)] px-6 py-5">
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-[var(--text-muted)]">Task status, error summary, and live logs</p>
            </div>
            <div className="flex items-center gap-2">
              {taskStatus ? (
              ...
              <tr>
                <td colSpan={7} className="px-4 py-24 text-center">
                  <div className="flex flex-col items-center justify-center space-y-3">
                    ...
                    <p className="text-xs text-[var(--text-muted)] max-w-sm">No accounts found for this platform. You can add manually or import in batch.</p>
                  </div>
                </td>
              </tr>
            )}
