...
# Force stdout/stderr to utf-8 (Windows defaults to gbk, which crashes on
# characters like ✗ ✓). errors="replace" replaces any failed chars with ?
# instead of throwing. Also set PYTHONUTF8 env var for child processes.
...
for _stream in (sys.stdout, sys.stderr):
    if _stream is not None and hasattr(_stream, "reconfigure"):
        try:
            ...
            pass
# Fallback: if reconfigure is unavailable (some PyInstaller versions), wrap with TextIOWrapper
...
# PyInstaller static analysis hooks — make modulefinder track Solver subprocess
# dependencies (quart etc.). Not executed at runtime, only for PyInstaller.
...
app = FastAPI(title="AIT Auto Register", version="2.0.0", lifespan=lifespan)
...
if __name__ == "__main__":
    ...
    # When the backend is spawned with --solver (PyInstaller packaged mode),
    # don't start the FastAPI main service, run as Turnstile Solver subprocess instead
