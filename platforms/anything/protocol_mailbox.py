...
class AnythingProtocolMailboxWorker:
    ...
    def run(
        ...
        *,
        ...
        raw = link_callback()
        if not raw:
            raise RuntimeError("Timed out getting Anything magic link")
        ...
        if organization_id:
            try:
                ...
                self.log(f"Auto-generate payment link failed, skipping: {exc}")
    ...
    def _resolve_magic_link(self, raw: str) -> str:
        ...
        if "/auth/magic-link" in candidate:
            return candidate
        self.log(f"Received email tracking link, attempting to resolve redirect: {candidate[:120]}")
    ...
    def _extract_magic_link(raw: str) -> dict[str, str]:
        ...
        if not match:
            ...
            raise RuntimeError(f"Unable to extract code from Anything magic link: {candidate[:200]}")
