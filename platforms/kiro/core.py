...
v10 = v8 core + HAR deep analysis fixes:
  * Core fix: removed _setup_signin_js_cookies() overwriting server-side cookies
    - workflow-csrf-token, directory-csrf-token etc. are managed by server Set-Cookie
    - only platform-ubid is generated by JS
  * _capture_cookies fix: cookies with Domain= are stored to bare domain to avoid duplicates
...
def encrypt_password_jwe(password, public_key_jwk):
    """* v10 fix: the browser encrypts not the bare password, but a JSON payload with JWT claims.
    ...
    # * Build JWT-like payload (consistent with browser app.js)
    now = int(time.time())
...
class KiroRegister:
    def __init__(self,proxy=None,tag="REG"):
        ...
        # * v10: only platform-ubid is generated by JS
        self._platform_ubid=_ubid()
        # * Data needed by Step 12 token flow
    ...
    def _capture_cookies(self,resp):
        ...
        * v10 fix: for Set-Cookie with Domain=, also store to bare domain
        to avoid .domain and bare domain each having a copy causing duplicate sends."""
        ...
        for k,v in resp.headers.multi_items():
            ...
            if dm:
                # Has Domain attribute: browser treats it as a domain cookie
    ...
    def _setup_signin_js_cookies(self, step_id=None):
        ...
        # platform-ubid: the only cookie truly generated by JS
    ...
    def _update_directory_csrf_with_signup(self):
        """* v10 new: After Step 4a, extract signupCsrfToken from server-returned
        workflow-csrf-token and update directory-csrf-token."""
        ...
        try:
            ...
            self.log(f"  Warning: failed to update directory-csrf-token: {e}")
    ...
    def _safe_cookie_list(self, domain_filter=None):
        """* v10: safely iterate cookies, avoiding CookieConflict."""
        result = []
        for c in self.s.cookies.jar:
    ...
    def _exec(self,step_id,inputs=None,prefix="",action_id=None,
        ...
        try: d=r.json()
        except: self.log(f"  X non-JSON: {r.text[:300]}"); return None
    ...
    def step1_kiro_init(self):
        ...
        if not redir: self.log(f"  X no redirectUrl: {d}"); return None
    ...
    def step2_get_wsh(self,redir_url):
        ...
        if not oid: self.log("  X no orchestrator_id"); return False
        # * Save orchestrator_id and callback_url (needed by Step 12)
        ...
        try:
            ...
            # * Save csrfToken (needed by Step 12a)
            ...
            if m:
                ...
                # * v10: after first loading login page, only set JS-generated platform-ubid
                self._setup_signin_js_cookies()
                return True
        except Exception as e: self.log(f"  X portal.sso: {e}")
        return False
    ...
    def step5_get_tes_token(self):
        self.log("Step 5: fetching TES token...")
    ...
    def step6_profile_load(self):
        self.log("Step 6: profile.aws page load...")
        if not self.profile_wf_id:
            self.log("  X no workflowID"); return None
    ...
    # ═══ Step 8: create-identity (OTP verification + identity creation) ═══
    def step8_create_identity(self, otp, email, full_name):
        ...
        if not reg_code or not sign_in_state:
            self.log(f"  X missing registrationCode or signInState")
            return None
    ...
    def step9_signup_registration(self, reg_code, sign_in_state):
        ...
        self._setup_signin_js_cookies()
        # Ensure awsccc cookie is on signin.aws domain (delete old first to avoid conflict)
        ...
        # * v8 debug: print cookies being sent (use _safe_cookie_list to avoid conflict)
        self.log("  cookies being sent (signin.aws):")
        ...
        if d.get("stepId") != "get-new-password-for-password-creation":
            self.log(f"  X expected get-new-password, got {d.get('stepId')}")
            return None
        self.log("  OK entered password setup step")
    ...
    def step10_set_password(self, pwd, email, step9_resp):
        ...
        if not pub_key:
            self.log("  X no public key, cannot encrypt password")
            return None
        self.log(f"  public key kid: {pub_key.get('kid')}")

        # * v10: no longer calling _setup_signin_js_cookies — let server Set-Cookie manage it
        # No longer manually manipulating aws-usi-authn path — _capture_cookies handles it correctly

        # * v10: clean up .domain duplicate cookies (curl_cffi internals may still create them)
        for c in list(self.s.cookies.jar):
        ...
        self.log(f"  OK JWE encryption done, length={len(jwe_password)}")
        req_id = _uuid()
        ...
        # These bypass _capture_cookies, so they circumvent our fix
        ...
        # * v10 debug: print final cookies (print full values for critical cookies)
        ...
        if d.get("stepId") != "end-of-user-registration-success":
            self.log(f"  X expected end-of-user-registration-success")
            return None
        # * Save workflowResultHandle (Step 12a needs it as authCode)
        ...
        self.log("  OK password set successfully, registration complete!")
        return d
    ...
    def step11_final_login(self, email, step10_resp):
        ...
        if not login_wsh or not state or not wf_result:
            self.log(f"  X redirect parameters incomplete")
            return None
        fwcim = self._gen_signin_fwcim()
        fp_i = {"input_type": "FingerPrintRequestInput", "fingerPrint": fwcim}
        usr_i = {"input_type": "UserRequestInput", "username": email}
        ...
        if d.get("stepId") == "end-of-workflow-success":
            self.log("  OK login succeeded! workflow complete!")
            # * Save state and workflowResultHandle from redirect URL (needed by Step 12a)
            ...
            if redir11:
                ...
                if wrh11:
                    ...
                    self.log(f"  * step11 workflowResultHandle={wrh11} (overwriting step10 value)")
        else:
            self.log(f"  Warning: stepId={d.get('stepId')}, may need additional steps")
        return d

    # ═══ Step 12: Kiro Web Portal OIDC Auth Code Flow → get accessToken + sessionToken ═══
    # * Old Device Auth flow is commented out (not applicable to view.awsapps.com SPA)
    # def step12_get_tokens_device_auth(self):
    #     """Old: OIDC Device Authorization flow (deprecated)
    #     Issue: view.awsapps.com is a SPA, device auth page needs independent SSO session,
    #     pure protocol cannot complete device auth confirmation. Switched to OIDC Auth Code Flow."""
    #     pass
    def step12_get_tokens(self):
        """Get tokens via Kiro Web Portal OIDC Authorization Code Flow.
        
        Real flow from HAR packet capture analysis:
        ...
        if not self._portal_csrf_token:
            self.log("  X missing portal csrfToken (step 2)")
            return None
        if not self._workflow_result_handle:
            self.log("  X missing workflowResultHandle (step 10)")
            return None
        if not self._step11_state:
            self.log("  X missing step11 state")
            return None
        ...
        self.log(f"  Status: {r.status_code}")
        if r.status_code != 200:
            self.log(f"  X sso-token failed: {r.status_code} {r.text[:500]}")
            return None
        sso_resp = r.json()
        bearer_token = sso_resp.get("token", "")
        sso_redirect = sso_resp.get("redirectUrl", "")
        if not bearer_token:
            self.log(f"  X no bearer token: {json.dumps(sso_resp, ensure_ascii=False)[:300]}")
            return None
        ...
        if sso_redirect and 'view.awsapps.com' in sso_redirect:
            # Remove fragment (#/...) and only GET the path part
        ...
        self.log(f"  Status: {r.status_code}")
        if r.status_code != 200:
            self.log(f"  X authentication_result failed: {r.status_code} {r.text[:500]}")
            return None
        ar_resp = r.json()
        auth_location = ar_resp.get("location", "")
        if not auth_location:
            self.log(f"  X no location: {json.dumps(ar_resp, ensure_ascii=False)[:300]}")
            return None
        ...
        if not redirect_loc:
            self.log(f"  X no redirect location, status={r.status_code}")
            return None
        ...
        if not auth_code:
            self.log(f"  X no code parameter in redirect URL")
            return None
        if not redirect_state:
            self.log(f"  Warning: no state parameter in redirect URL, falling back to self.state")
        ...
        self.log(f"  Status: {r.status_code}")
        if r.status_code != 200:
            self.log(f"  X ExchangeToken failed: {r.status_code}")
    ...
    # SPA OIDC class (class v) device auth API:
    ...
    # SPA calls via view.awsapps.com/api/oidc/ proxy, authenticated with SSO session cookie
    # We call portal.sso directly with bearer token authentication
    def step12f_device_auth(self, bearer_token):
        ...
        12h: Simulate SPA device auth confirmation (call portal.sso API with bearer token):
        ...
        self.log(f"  Status: {r.status_code}")
        if r.status_code != 200:
            self.log(f"  X client/register failed: {r.text[:300]}")
            return None
        reg_resp = r.json()
        client_id = reg_resp.get("clientId", "")
        client_secret = reg_resp.get("clientSecret", "")
        if not client_id or not client_secret:
            self.log(f"  X no clientId/clientSecret")
            return None
        ...
        self.log(f"  Status: {r.status_code}")
        if r.status_code != 200:
            self.log(f"  X device_authorization failed: {r.text[:300]}")
            return None
        da_resp = r.json()
        ...
        if not device_code or not user_code:
            self.log(f"  X no deviceCode/userCode")
            return None
        self.log(f"  OK userCode={user_code}")
        self.log(f"  OK verificationUri={verification_uri[:100]}...")

        # ── 12h: Device auth confirmation (call oidc.amazonaws.com directly) ──
        # Real flow (from browser packet capture):
        ...
        self.log("  12h: Device auth confirmation (call oidc.amazonaws.com directly)...")
        ...
        if r.status_code != 200:
            self.log(f"  X accept_user_code failed")
            return None
        accept_resp = r.json()
        ...
        if r.status_code not in (200, 204):
            self.log(f"  X associate_token failed")
            return None
        ...
        while time.time() - poll_start < poll_timeout:
            ...
            try:
                ...
                if err_code == "authorization_pending":
                    ...
                    self.log(f"  X token error: {err_code} - {err.get('error_description','')}")
                    return None
            except:
                self.log(f"  X token response abnormal: {r.status_code} {r.text[:200]}")
                return None
            time.sleep(poll_interval)

        if not oidc_token:
            self.log("  X token polling timeout")
            return None
...
# ═══════════════════════════════════════════
#  TechFlow email (deprecated, kept as comments)
# ═══════════════════════════════════════════
#
...
#         time.sleep(3)
#     print(f"{prefix}  X verification code timeout")
#     return None

# ═══════════════════════════════════════════
#  qqemail.eu.org temporary email API (deprecated, kept as comments)
# ═══════════════════════════════════════════
...
#
# def create_qqemail():
#     """Create qqemail temporary mailbox, returns (email, inbox_id)"""
#     chars = string.ascii_lowercase + string.digits
#     local = "".join(random.choice(chars) for _ in range(random.randint(8, 13)))
#     h = _qqemail_headers()
...
def wait_for_otp(account_id=None, timeout=120, tag=""):
    ...
    while time.time() - start < timeout:
        try:
            ...
            print(f"{prefix}  Warning: email query exception: {e}")
        elapsed = int(time.time() - start)
        print(f"{prefix}  Waiting... ({elapsed}s/{timeout}s)")
        time.sleep(3)
    print(f"{prefix}  X verification code timeout")
...
def main():
    ...
    pwd = input("Password (leave empty for auto-generation): ").strip() or None
