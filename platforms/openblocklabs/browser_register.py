...
def _click_turnstile_in_iframe(page, log_fn=print) -> bool:
    ...
    try:
        ...
        log_fn(f"In-frame click failed: {exc}")
...
def _handle_turnstile(page, log_fn=print, wait_secs: int = 12) -> bool:
    ...
    if not solved:
        log_fn("Auto-click failed, waiting for manual pass (up to 90 seconds)...")
...
def _wait_for_visible_element(page, selectors: list[str], timeout: int = 15, *, require_enabled: bool = False):
    ...
    while time.time() < deadline:
        ...
        if el:
            return el, used_sel
        time.sleep(0.2)
    raise RuntimeError(f"No visible element found: {' | '.join(selectors)}")
...
def _advance_to_email_verification(page, btn_selectors: list[str], log_fn=print, timeout: int = 40) -> None:
        ...
        if now - last_click_at >= 1.5:
            ...
            if btn_el:
                ...
                try:
                    ...
                    log_fn(f"Detected intermediate page, clicking button: {used_sel}")
                    last_click_at = now
                    time.sleep(2)
                    continue
                except Exception:
    ...
    raise RuntimeError(f"Did not reach verification code page: {page.url}")
...
def _wait_for_signup_session(page, timeout: int = 30) -> str:
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            pass
        time.sleep(0.5)
    raise RuntimeError(f"Timed out waiting for authorization_session_id: {page.url}")
...
""")
            ...
            if not self.otp_callback:
                raise RuntimeError("OpenBlockLabs registration requires email verification code but no otp_callback provided")
