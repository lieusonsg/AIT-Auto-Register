...
The Windsurf website HAR primarily uses two types of API:
  1. /_devin-auth/* JSON API for email verification code registration/login
  2. /_backend/exa.* application/proto API for querying user, plan, and quota

This does not depend on full protobuf generated code; only implements lightweight
...
class WindsurfClient:
    ...
    def login_with_password(self, email: str, password: str) -> dict[str, str]:
        """Login to an existing account with email + password, returns session_token etc."""
        self.log(f"Logging in to Windsurf with password: {email}")
