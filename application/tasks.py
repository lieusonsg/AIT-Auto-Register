...
class TaskLogger:
    ...
    def mark_running(self) -> None:
        ...
        self.log("Task started", event_type="state")
...
def _auto_push_any2api(task_logger: TaskLogger, account) -> None:
    """Auto-push account to Any2API after registration (if configured)."""
    try:
...
def _auto_followup_windsurf_payment(
    *,
    ...
    if use_browser:
        ...
        if turnstile_token:
            params["turnstile_token"] = turnstile_token
    logger.log("Registration succeeded, auto-generating Windsurf Pro Trial Stripe link")
    try:
...
def _execute_register_task(payload: dict[str, Any], logger: TaskLogger) -> None:
    ...
    if herosms_enabled:
        logger.log(
            f"HeroSMS mode: target {target_success} successes, auto-retry on failure, "
            f"up to {hero_extra_max} extra successes when number is still reusable"
        )
    ...
    def _do_one(index: int) -> bool | str:
        ...
        try:
            logger.log(f"Starting registration {index + 1}/{count}")
            if resolved_proxy:
            ...
            logger.log(f"✓ Registration succeeded: {account.email}")
            ...
            try:
                ...
                if alive:
                    logger.log(
                        ...
                        f"{int(info.get('use_count') or 0)} successes"
                    )
                return bool(alive)
            except Exception:
                return False
